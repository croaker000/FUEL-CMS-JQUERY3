...
//		this.previewPath = myMarkItUpSettings.previewParserPath;
	...
	_initLeftMenu : function(){
		...
		var leftNavTogglers = function(id, index){
         $('#' + id + ' h3').on('click', {id:id,index:index},
				function(e){
	...
	_initTopMenu : function(){
      $('#topnav li').on('mouseenter mouseleave',function(e){
			$('ul', this).show();
		}, 	function(e){
				$('ul', this).hide();
		});
	...
	items : function(){
      ...
      $('#search_term').trigger('focus');
		...
		if ($('#tree').exists()){
            ...
            $('#toggle_tree').trigger('click');
			} else {
				$('#toggle_list').trigger('click');
			}

			// bind keyboard shortcuts
         $(document).on('keydown', jqx.config.keyboardShortcuts.toggle_view, function(e){
      ...
      $('#multi_delete').on('click',function(){
         $('#toggle_list').off('click');
			var deleteUrl = _this.modulePath + '/delete';
			$('#form').attr('action', deleteUrl).attr('method', 'post').trigger('submit');
			return false;
		});
		...
		$('.more_filters select').not('[multiple]').on('change',function(e){
		...
		$('#export_data').on('click',function(e){
			...
			$('#form').attr('action', _this.modulePath + '/export').attr('method', 'post').trigger('submit');
		...
		$('#adv-search-btn, #adv-search-close').on('click',function(e){
			e.preventDefault();
			$('.adv_search').toggle();
		});

		$('.filters_toggle').toggleClick(
			function(){
				var $this = $(this);
	...
	add_edit : function(initSpecFields){
		...
		$('.publish_action').on('click',function(e){
			$.removeChecksave();
		...
		$('.unpublish_action').on('click',function(e){
			$.removeChecksave();
			if ($('#published:checkbox').length > 0) {
		...
		$('.duplicate_action').on('click',function(e){
			$('#form').attr('action', _this.modulePath + '/duplicate?inline=' + $('#fuel_inline').val()).trigger('submit');
			return false;
		});

		$('.replace_action').on('click',function(e){
			var url = $(this).attr('href');
			html = '<iframe id="replace_iframe" src="' + url + '"></iframe>';
			$modal = fuel.modalWindow(html, 'replace_modal', true);

         $modal.find('iframe#replace_iframe').on('load', function(){
				var iframeContext = this.contentDocument;
				var replacedId = parseInt($('#new_fuel_replace_id', iframeContext).val());

				$('#form', iframeContext).on('submit',function(){
		...
		$('#fuel_restore_version').on('change',function(e){
			$.removeChecksave();
			if ($(this).val() != ''){
		...
		$('#fuel_other_items').on('change',function(e){
			$.removeChecksave();
			if ($(this).val() != ''){
		...
		// keyboard shortcuts
      $(document).on('keydown', jqx.config.keyboardShortcuts.save, function(e){
         $('.save').trigger('click');
			return false;
		});

		$(document).on('keydown', jqx.config.keyboardShortcuts.view, function(e){
			window.location = ($('.key_view_action').attr('href'));
		});

		//$('#form input:first').select();
      //$(':input', '#form').filter(':first').focus();
      $(':input', '#form').filter(':first').trigger('focus');
	...
	initSpecialFields : function(context){
      ...
      $('#form input:first', context).trigger('select');
	},
	...
	_initFormTabs : function(context){
		if (!$('#fuel_form_tabs', context).length) {
			...
			$('.fieldset-grouped').each(function(idx,context){
				...
				$tabs.on('tabClicked', tabCallback);
	...
	_initViewPage : function(){
		...
		$('.view_action').on('click',function(e){
			...
			$('#viewpage_close').on('click',function(){
	...
	tableCallback : function(_this){
		...
		$("#data_table td[class^='col']").each(function(){
			$(".publish_action", this).on('click',function(e){
